...
app.post('/categories', authenticateToken, isAdmin, (req, res) => {
  ...
  db.run('INSERT INTO categories (name, department_id) VALUES (?, ?)', [name, department_id], function(err) {
    if (err) {
...
app.put('/categories/:id', authenticateToken, isAdmin, (req, res) => {
  const { name, department_id } = req.body;
  db.run('UPDATE categories SET name = ?, department_id = ? WHERE id = ?', [name, department_id, req.params.id], function(err) {
    if (err) {
...
app.post('/assets', authenticateToken, isAdmin, (req, res) => {
  const { item_code, date_acquired, serial_no, unit_price, description, supplier, sub_category_id } = req.body;
  db.run(
    'INSERT INTO assets (item_code, date_acquired, serial_no, unit_price, description, supplier, sub_category_id) VALUES (?, ?, ?, ?, ?, ?, ?)',
    [item_code, date_acquired, serial_no, unit_price, description, supplier, sub_category_id],
    function(err) {
      if (err) {
...
app.put('/assets/:id', authenticateToken, isAdmin, (req, res) => {
  const { item_code, date_acquired, serial_no, unit_price, description, supplier, sub_category_id } = req.body;
  db.run(
    'UPDATE assets SET item_code = ?, date_acquired = ?, serial_no = ?, unit_price = ?, description = ?, supplier = ?, sub_category_id = ? WHERE id = ?',
    [item_code, date_acquired, serial_no, unit_price, description, supplier, sub_category_id, req.params.id],
    function(err) {
      if (err) {
...
app.post('/notifications/broadcast', authenticateToken, (req, res) => {
  ...
  db.all('SELECT id FROM users', [], (err, users) => {
    if (err) {
      console.error('Error fetching users for notification broadcast:', err.message);
    ...
    db.serialize(() => {
      ...
      users.forEach(user => {
        stmt.run(user.id, ticket_id, message, (err) => {
          if (err) {
            console.error(`Error inserting notification for user ${user.id}:`, err.message);
            // Decide if you want to rollback on single failure
          }
        });
      });
      db.run('COMMIT', (err) => {
        if (err) {
          console.error('Failed to commit notification broadcast transaction:', err.message);
          return res.status(500).json({ message: 'Failed to broadcast notifications.' });
        }
        stmt.finalize();
        res.status(201).json({ message: 'Notifications broadcasted successfully.' });
      });
    });
  });
});
